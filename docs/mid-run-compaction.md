...
- **Fail-closed shapes**: only known Pi compact shapes are recognized — `compact()` must call `this.abort()` exactly once, reference `appendCompaction`, and repoint `agent.state.messages` at the finalized context, either inline (0.81 legacy, 0.84 connected-listener, 0.85/0.86) or through a prototype helper one call level deep (0.87's `_refreshFinalizedContext()`). Internal drift → `InlineCompactionUnavailableError`, the current run stays alive, and the mode degrades (see matrix below). It never falls back to the abort + `blackhole-resume` path. The rest of 0.87's surface is not yet supported — see [#118](https://github.com/k0valik/pi-blackhole/issues/118).
