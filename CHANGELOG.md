...
- **Pi 0.87 is held out of the automatic dependency bump.** Dependabot ignores `@earendil-works/*` `>=0.87.0` so the toolchain settles on 0.86.x. The compact-shape guard now follows 0.87's helper indirection (see Fixed), but the bump still fails typecheck and four auto-compaction eligibility tests against 0.87.0 ([#118](https://github.com/k0valik/pi-blackhole/issues/118)). `peerDependencies` (`>=0.85.1 <1.0.0`) is unchanged; on 0.87 the opt-in `resume` mode is expected to work while the remaining 0.87 gaps are open.
